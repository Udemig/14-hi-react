...
const TextContainer = () => {
  const { textToTranslate, translatedText, loading, sourceLang, targetLang } = useSelector((store) => store.translate);
  ...
  const handleSpeakSource = () => {
    ...
    // SpeechSynthesisUtterance: seslendirelecek metni ve ayalarını tutan bir nesne oluşturur
    const uttarance = new SpeechSynthesisUtterance(textToTranslate);

    // uttarance.lang: hangi dilde/aksasnda konuşulucağını belir
    if (sourceLang.value) {
