...
function segmentFromCar(car: CarInfo): { segment: string; base: number } {
  ...
  const luxuryBrands = ["bmw", "mercedes", "audi", "porsche", "lexus", "jaguar", "land rover", "range rover"];
  const isLuxuryBrand = luxuryBrands.some((b) => make.includes(b));

  const isFlagship =
    (make.includes("bmw") && (model.includes("750") || basemodel.includes("7 series"))) ||
    (make.includes("mercedes") && (basemodel.includes("s") || model.includes("s"))) ||
    (make.includes("audi") && (basemodel.includes("a8") || model.includes("a8")));

  const isSUV =
    vclass.includes("suv") || basemodel.includes("x") || basemodel.includes("gle") || basemodel.includes("q");
  ...
  if (isLuxuryBrand && (isLarge || isPowerful)) return { segment: "luxury", base: 5600 };
 ...
 */
export function getRentalPrice(car: CarInfo, opts?: { currentYear?: number }): DailyRentalPriceTR {
  const notes: string[] = [];
  ...
  // 7) Çekiş (drive)
  const driveType = normalizeDrive(typeof car.drive === "string" ? car.drive : (car.drive as string | undefined));
  const driveMult = driveType ? DRIVE_MULT[driveType] : 1.0;
  if (!driveType && car.drive) notes.push(`Bilinmeyen drive değeri: "${car.drive}" -> 1.00 uygulandı.`);
  ...
  // fiyat
  let price = base * ageMult * displMult * cylMult * co2Mult * fuelMult * tranyMult * driveMult * marketMult;

  // Segment bazlı mantık sınırları
  const min = segment.startsWith("luxury") ? 3500 : segment.startsWith("premium") ? 2200 : 1200;

  const max = segment.startsWith("luxury") ? 30000 : segment.startsWith("premium") ? 15000 : 9000;
